          ...
          <div v-else>
            <div class="running-state">
              <CheckmarkFilled16
                ...
                class="icon-running"
              />
              <WarningFilled16 v-else class="icon-not-running" />
              <span>{{
                config.running
                  ? $t("status.running")
                  : $t("status.not_running")
              }}</span>
            </div>
            <div v-if="!config.host" class="access-help">
            ...
            <div v-else class="access-buttons">
              <NsButton
                ...
                >{{ $t("status.open_odoo") }}</NsButton
              >
              <NsButton
                kind="tertiary"
                :icon="Launch20"
                @click="openUrl(config.database_manager_url)"
                >{{ $t("status.database_manager") }}</NsButton
              >
            </div>
            <NsInlineNotification
              kind="info"
              :title="$t('status.first_run_title')"
              :description="$t('status.first_run_desc')"
              :showCloseButton="false"
              class="first-run"
            />
            <div class="master-password-note">
              {{ $t("status.master_password_note") }}
            </div>
          </div>
        </cv-tile>
      </cv-column>
